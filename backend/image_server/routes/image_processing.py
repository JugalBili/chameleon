...
async def generate_image(
    ...
    image_response: GetImageResponse = image_repository.get_raw_image_by_hash(
        image_data.uid, image_data.raw_image_hash, True
    )
    if image_response == None:
        pass

    json_data = {}
    image_json_response: GetJSONResponse = image_repository.get_json_by_hash(
        image_data.uid, image_data.raw_image_hash
    )
    ...
    if json_data and json_data["masks"]:
        mask_responses: list[GetMaskResponse] = image_repository.get_masks_by_hash(
            image_data.uid, image_data.raw_image_hash, json_data["masks"]
        )
        ...
        rgb_colors = [
            [color.rgb.r, color.rgb.g, color.rgb.b] for color in image_data.colors
        ]
        masks, colored_images = ds_instance.run_pipeline(
            image_cv, image_data.raw_image_hash, rgb_colors
        )
    ...
    if not json_data:
        ...
        mask_hashes = await image_repository.upload_masks(
            image_data.uid, image_data.raw_image_hash, bmp_buffers
        )
    ...
    await image_repository.upload_json(
        image_data.uid, image_data.raw_image_hash, json_data
    )
    response = []

    for i in range(len(colored_images)):
        _, image_bytes = cv2.imencode(".jpg", colored_images[i])
        processed_image_hash = await image_repository.upload_processed_image(
            image_data.uid,
            image_data.raw_image_hash,
            image_bytes.tobytes(),
            image_data.colors[i],
        )
        response.append(
