...
class GalleryRepository:
    ...
    def get_review_image(
        self, image_hash: str, paint_id: str, download_image: bool = False
    ) -> None | GetReviewImageResponse:
        base_path = f"{self.gallery_folder}/{paint_id}"
    ...
    async def get_paint_reviews(self, paint_id: str):
        ...
        async for review in review_docs:
            try:
                ...
                raise HTTPException(
                    status_code=500, detail=f"Error parsing user review: {e}"
                )
    ...
    async def add_paint_review(self, review_dto: ReviewDto, user: User):
        review_ref = (
            self.collectionRef.document(review_dto.paint_id)
            .collection("reviews")
            .document(user.uid)
        )
        ...
        if review_doc.exists:
            ...
            review = Review(
                paint_id=review_dto.paint_id,
                uid=user.uid,
                review=review_dto.review,
                image_hashes=review_dto.image_hashes,
